...
cleanup() {
    ...
    # Do not remove modal credentials on exit anymore
...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    # Check if userData.json already exists. If so, skip the login process.
    if [ -f "modal-login/temp-data/userData.json" ]; then
        echo_green ">> Found existing login credentials. Skipping login."
